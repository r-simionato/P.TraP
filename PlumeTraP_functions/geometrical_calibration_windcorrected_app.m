%% geometrical_calibration_windcorrected_app - PlumeTraP
% Function to apply the geometric calbration and the wind correction with
% input from GUI
% Author: Riccardo Simionato. Date: April 2024
% Structure: PlumeTraP --> geometrical_calibration_windcorrected_app

function [pixel,wdir] = geometrical_calibration_windcorrected_app(procframes,...
    outFolder_proc,imageList_proc,outFolder_parameters,...
    imgplume_height,imgplume_width,par,geopot_nc,wind_nc,pixel,roiPos)
...
if isnan(par.wind)
    ...
    ylim([0 max(height_ventlevel)+50])
    xlabel('Wind direction [°]')
    ylabel('Height [m]')
    legend({'wind values','average wind direction'},'Location','southoutside',...
        'FontSize',6)
end
